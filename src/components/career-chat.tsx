...
import { type StoredChatMessage, useChatStore } from '@/stores/chatStore';
...
export function CareerChat() {
  ...
  useEffect(() => {
    ...
    if (
      JSON.stringify(storedMessages) !== JSON.stringify(activeSession.messages)
    ) {
      updateActiveMessages(storedMessages);
    }
  }, [activeSession, messages, updateActiveMessages]);
  ...
  return (
    <div className='flex h-full min-h-0 w-full flex-1 p-4'>
      <Card className='flex h-full w-full min-w-0 flex-col border-border/80 bg-card/80 backdrop-blur-sm'>
        <CardHeader className='relative border-b border-border/60 pb-4 flex-shrink-0'>
        ...
        <CardContent className='flex flex-1 flex-col overflow-hidden p-0'>
          ...
          {error && (
            <p
              className='px-4 py-2 text-sm text-destructive border-t border-border/60'
              role='alert'
            >
              {error}
            </p>
