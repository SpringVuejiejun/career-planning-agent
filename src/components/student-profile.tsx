...
export function StudentProfilePage() {
  ...
  const latestSummary = useMemo(() => {
    ...
    const edu = (latest.education && typeof latest.education === "object") ? latest.education : null
    ...
    const comps = (latest.competencies && typeof latest.competencies === "object") ? latest.competencies : {}
    ...
    const internship = (latest.internship && typeof latest.internship === "object") ? latest.internship : null
    const internshipSummary =
      internship?.["实习经历概述"] || internship?.summary || ""
    const internshipRole =
      internship?.["岗位"] || internship?.role || ""
    const internshipAchievement =
      internship?.["成果"] || internship?.achievement || ""
    const internshipDuration =
      internship?.["时长"] || internship?.duration || ""
    const internshipLine = [internshipSummary, internshipRole, internshipAchievement, internshipDuration]
      .filter(Boolean)
      .join("；")

    const scoring = (latest.scoring_detail && typeof latest.scoring_detail === "object") ? latest.scoring_detail : null
    ...
    const parts: string[] = []
    parts.push(`总体评分：完整度 ${latest.completeness_score ?? "—"}（${scoreBadge(latest.completeness_score)}），竞争力 ${latest.competitiveness_score ?? "—"}（${scoreBadge(latest.competitiveness_score)}）。`)
    if (eduLine) parts.push(`教育背景：${eduLine}。`)
    parts.push(`专业技能：${skills.length ? skills.slice(0, 12).join("、") : "暂未明确（建议补充技能栈与熟练度）"}。`)
    parts.push(`证书情况：${certs.length ? certs.slice(0, 10).join("、") : "暂未提及（如有请补充）"}。`)
    parts.push(`项目经历：${projects.length ? `已记录 ${projects.length} 项（建议补充项目职责与成果）` : "暂未提及（建议补充 1-2 个代表项目）"}。`)
    parts.push(`实习经历：${internshipLine ? internshipLine : "暂未提及（建议补充岗位、周期与可量化成果）"}。`)
    if (compLines.length) parts.push(`通用素质亮点：${compLines.join("；")}。`)
    parts.push(`竞赛/奖项：${awards.length ? awards.slice(0, 10).join("、") : "暂未提及"}。`)
    ...
    <div className="flex h-full w-full min-h-0 flex-1 p-4">
      <Card className="flex h-full w-full min-w-0 flex-col border-border/80 bg-card/80 backdrop-blur-sm">
        <CardHeader className="border-b border-border/60 pb-4 flex-shrink-0">
          ...
          <CardDescription>
            通过“录入信息”或“简历上传（推荐 txt）”生成能力画像，并给出完整度/竞争力评分。
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="flex flex-1 min-h-0 gap-4 overflow-hidden p-4">
          <div className="w-[45%] min-w-[320px] overflow-y-auto pr-2 space-y-4">
            ...
            <div className="rounded-lg border border-border/60 bg-muted/40 p-3">
              ...
              <div className="mt-2 flex justify-end">
                <Button onClick={() => void createFromText()} disabled={loading || text.trim().length < 10}>
                  生成画像
                </Button>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-border/60 bg-muted/40 p-3">
              ...
              <div className="mt-2 text-xs text-muted-foreground">
                支持 <span className="font-medium">txt/pdf/docx</span> 上传；若文件内容复杂导致解析不完整，可把简历文本粘贴到下方补充说明提高准确性。
              </div>
              <Input
                type="file"
              ...
              />
              <div className="mt-2 flex justify-end">
                <Button onClick={() => void createFromResume()} disabled={loading || !file}>
                  上传并生成
                </Button>
              </div>
            </div>
          ...
          <div className="flex-1 min-w-0 overflow-y-auto pl-2">
            <div className="mb-2 text-sm font-semibold text-foreground/80">历史画像</div>
            <div className="space-y-2">
              {profiles.map((p) => (
                <div key={p.id} className="rounded-lg border border-border/60 bg-muted/40 p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="ml-auto flex gap-2 text-xs">
                      <span className="rounded bg-background px-2 py-0.5 text-muted-foreground">
                        完整度：{p.completeness_score ?? "—"}（{scoreBadge(p.completeness_score)}）
                      </span>
                      <span className="rounded bg-background px-2 py-0.5 text-muted-foreground">
                        竞争力：{p.competitiveness_score ?? "—"}（{scoreBadge(p.competitiveness_score)}）
                      </span>
                    </div>
                  </div>
                  <div className="mt-2 text-xs text-muted-foreground">
                    <span className="font-medium text-foreground/70">技能：</span>
                    {Array.isArray(p.skills) ? p.skills.slice(0, 12).join("、") : "—"}
                  </div>
                  <div className="mt-1 text-xs text-muted-foreground">
                    <span className="font-medium text-foreground/70">证书：</span>
                    {Array.isArray(p.certificates) ? p.certificates.slice(0, 8).join("、") : "—"}
                  </div>
                </div>
              ))}
              {profiles.length === 0 && (
                <div className="text-sm text-muted-foreground">暂无画像，请先在左侧生成一份。</div>
              )}
            </div>

            {latest && (
              <div className="mt-4 rounded-lg border border-border/60 bg-muted/40 p-3">
                <div className="text-sm font-semibold">最新画像详情（摘要）</div>
